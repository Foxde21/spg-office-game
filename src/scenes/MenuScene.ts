...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(width / 2, height / 2 - 60, 'Welcome to Office Quest', TEXT_STYLE_TITLE).setOrigin(0.5)
    this.add.text(width / 2, height / 2 - 20, 'Path from Junior to Team Lead without burning out', TEXT_STYLE_SUB).setOrigin(0.5)
    ...
    const continueBtn = this.add.text(width / 2, height / 2 + 60, 'Продолжить', TEXT_STYLE_BUTTON).setOrigin(0.5).setInteractive({ useHandCursor: true })
