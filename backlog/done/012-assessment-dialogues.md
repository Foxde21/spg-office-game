...
5. При выборе:
   ...
   - Если сессия не закончена — показать следующий вопрос (`assessmentNext`)
   - Если закончена — показать итоговое резюме (`assessmentFinish`)
...
- Если вопросы по домену кончились, игрок может:
  - выбрать другой домен (открыть домен-селект с resume)
  - пройти домен ещё раз (reset прогресса домена)
...
- AI-ветка должна быть выбрана (career path `ai` + flag `careerPathChosen`) для доступа к ассесмент-диалогам.
